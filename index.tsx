...
const PokerApp = () => {
  ...
  const [mediaItems, setMediaItems] = useState<SessionMedia[]>([]);
  ...
  const sendToLab = (session: Session) => {
    // Feature implementation: Append tags to the beginning of the prompt
...
*Analyze my performance. Focus on GTO adherence.*`;
  ...
  const saveLabState = () => {
    const item: LabHistoryItem = {
      id: `saved-${Date.now()}`,
      timestamp: `SAVED: ${new Date().toLocaleString()}`,
      prompt: labPrompt,
      response: aiResponse || '',
      strategicTags: labStrategicTags,
      sizingAdvice: labSizingAdvice || undefined,
  ...
  const handleDeepAnalysis = async (mode: 'thinking' | 'search' | 'maps' | 'sizing') => {
    ...
    try {
      ...
      parts.push({ text: labPrompt || "Analyze this situation." });

      const responseSchema = {
        type: Type.OBJECT,
        properties: {
          analysis: { type: Type.STRING },
          strategicTags: { type: Type.ARRAY, items: { type: Type.STRING } },
      ...
      const config: any = {
        systemInstruction: "World-class GTO poker coach. Extract strategic actions and sizing from the hand history text.",
               ...
               {activeSession ? (
                 <div className="flex items-center gap-4 bg-emerald-500/10 border border-emerald-500/30 px-6 py-3 rounded-2xl">
                    ...
                    <div className="flex gap-2 ml-4">
                      ...
                      <button onClick={() => {
                        const amount = prompt("Enter profit update (e.g. 50 or -20):");
                        if (amount) setActiveSession({...activeSession, currentProfit: activeSession.currentProfit + parseFloat(amount)});
                      }} className="p-2 bg-gray-800 rounded-lg border border-gray-700 hover:bg-gray-700"><DollarSign size={16}/></button>
                    </div>
                 </div>
               ) : (
          ...
          <section className="max-w-4xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={saveLabState} className="flex items-center gap-2 px-4 py-2 bg-emerald-600/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-600/20 rounded-xl text-xs font-bold uppercase transition-all">
                ...
                <button onClick={() => setShowLabHistory(!showLabHistory)} className="flex items-center gap-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-xl text-xs font-bold uppercase transition-all">
                  <FolderOpen size={16} /> Load {labHistory.length > 0 && `(${labHistory.length})`}
                </button>
              </div>
            </div>
            ...
            {showLabHistory && (
              <div className="bg-[#161b22] border border-gray-800 rounded-3xl p-6 mb-6 space-y-4 animate-in slide-in-from-top-2">
                <div className="max-h-64 overflow-y-auto space-y-3 pr-2">
                  {labHistory.length === 0 ? (
                    <p className="text-center py-8 text-gray-600 text-xs italic">Empty archives.</p>
                  ) : labHistory.map(item => (
                    <div key={item.id} className="p-4 bg-gray-900/50 rounded-2xl border border-gray-800 hover:border-emerald-500/30 cursor-pointer transition-all" 
                      onClick={() => { 
                        setLabPrompt(item.prompt); 
                        setAiResponse(item.response); 
                        setLabStrategicTags(item.strategicTags || []);
                        setLabSizingAdvice(item.sizingAdvice || null);
                        setLabMedia(item.media || null);
            ...
            <div className="bg-[#161b22] rounded-3xl border border-gray-800 shadow-xl overflow-hidden">
              ...
              <div className="bg-gray-900/50 p-2 border-b border-gray-800 flex flex-wrap items-center gap-1">
                {/* General Formatting */}
                <button onClick={() => insertFormatting('**', '**')} title="Bold" className="p-2 hover:bg-gray-800 rounded-lg text-gray-400 hover:text-white transition-colors"><Bold size={16} /></button>
                <button onClick={() => insertFormatting('*', '*')} title="Italic" className="p-2 hover:bg-gray-800 rounded-lg text-gray-400 hover:text-white transition-colors"><Italic size={16} /></button>
                <div className="h-6 w-px bg-gray-800 mx-1" />
                
                {/* Street Identifiers */}
                <div className="flex gap-1 mr-1">
                  <button onClick={() => insertFormatting('### PRE-FLOP\n')} className="px-2 py-1 bg-gray-800 hover:bg-emerald-600/30 text-[10px] font-black uppercase text-gray-400 hover:text-emerald-400 rounded-md transition-all">Pre</button>
                  <button onClick={() => insertFormatting('### FLOP: [Ad 2h 7s]\n')} className="px-2 py-1 bg-gray-800 hover:bg-emerald-600/30 text-[10px] font-black uppercase text-gray-400 hover:text-emerald-400 rounded-md transition-all">Flop</button>
                  <button onClick={() => insertFormatting('### TURN: [Kh]\n')} className="px-2 py-1 bg-gray-800 hover:bg-emerald-600/30 text-[10px] font-black uppercase text-gray-400 hover:text-emerald-400 rounded-md transition-all">Turn</button>
                  <button onClick={() => insertFormatting('### RIVER: [Qs]\n')} className="px-2 py-1 bg-gray-800 hover:bg-emerald-600/30 text-[10px] font-black uppercase text-gray-400 hover:text-emerald-400 rounded-md transition-all">River</button>
                </div>
                ...
                {/* Action Markers */}
                <div className="flex gap-1 mr-1">
                  <button onClick={() => insertFormatting('>>> HERO [CHECK]')} className="px-2 py-1 bg-blue-900/20 border border-blue-500/20 text-blue-400 hover:bg-blue-900/40 text-[10px] font-black uppercase rounded-md transition-all">Check</button>
                  <button onClick={() => insertFormatting('>>> HERO [BET 50%]')} className="px-2 py-1 bg-emerald-900/20 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-900/40 text-[10px] font-black uppercase rounded-md transition-all">Bet</button>
                  <button onClick={() => insertFormatting('>>> HERO [RAISE 3X]')} className="px-2 py-1 bg-purple-900/20 border border-purple-500/20 text-purple-400 hover:bg-purple-900/40 text-[10px] font-black uppercase rounded-md transition-all">Raise</button>
                  <button onClick={() => insertFormatting('>>> HERO [FOLD]')} className="px-2 py-1 bg-red-900/20 border border-red-500/20 text-red-400 hover:bg-red-900/40 text-[10px] font-black uppercase rounded-md transition-all">Fold</button>
                </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {['SB', 'BB', 'UTG', 'HJ', 'CO', 'BTN'].map(pos => (
                    <button key={pos} onClick={() => insertFormatting(`[${pos}] `)} className="px-1.5 py-0.5 bg-gray-800 hover:bg-gray-700 text-[9px] font-black text-gray-500 hover:text-gray-300 rounded transition-all">{pos}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="relative p-1">
                <textarea 
                  ...
                  placeholder="Record your hand history here. Use the toolbar above to structure your data for the AI Solver..." 
                  className="w-full bg-[#0d0f12] border-none rounded-2xl p-6 h-72 focus:ring-0 outline-none resize-none transition-all placeholder:text-gray-700 font-mono text-sm leading-relaxed" 
                />
                {labMedia && (
                ...
                <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between">
                  <label className="p-3 bg-gray-800/80 backdrop-blur rounded-xl cursor-pointer hover:bg-gray-700 transition-colors text-gray-400 flex items-center gap-2 border border-gray-700 shadow-lg">
                    <ImageIcon size={18} />
                    <span className="text-xs font-bold uppercase tracking-tighter">Attach Lab Media</span>
                  ...
                  <div className="flex items-center gap-2 text-gray-700 text-[10px] font-bold uppercase tracking-widest bg-black/20 px-3 py-1.5 rounded-full">
                    <Hash size={12}/> Structure helps analysis
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 bg-[#161b22] p-6 rounded-3xl border border-gray-800">
                    <h3 className="text-xs font-black uppercase tracking-widest text-emerald-500 mb-4 flex items-center gap-2"><TagIcon size={14}/> Strategic Identifiers</h3>
                    <div className="flex flex-wrap gap-2">
                      {labStrategicTags.map((tag, idx) => <span key={idx} className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-[10px] font-bold uppercase">{tag}</span>)}
                    </div>
                  </div>
                  <div className="bg-[#161b22] p-6 rounded-3xl border border-gray-800 text-center flex flex-col justify-center">
                    <h3 className="text-xs font-black uppercase tracking-widest text-indigo-400 mb-2">Optimal Bet</h3>
                  ...
                  <div onClick={() => setExpandedSessionId(expandedSessionId === s.id ? null : s.id)} className={`bg-[#161b22] border ${expandedSessionId === s.id ? 'border-emerald-500/50 rounded-t-2xl' : 'border-gray-800 rounded-2xl'} p-5 flex items-center justify-between cursor-pointer hover:bg-gray-800/40 transition-all`}>
                    ...
                    <div className="flex items-center gap-3">
                      {s.mediaItems.length > 0 && <div className="flex -space-x-2">
                        {s.mediaItems.map(m => (
                          <div key={m.id} className="w-6 h-6 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-[8px] text-emerald-400" title={m.category}>
                            {m.category[0]}
                          </div>
                        ))}
                      </div>}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="bg-[#0d0f12] border border-gray-800 rounded-xl p-4">
                            <h4 className="text-[10px] font-black uppercase text-gray-500 mb-3">Granular Media ({s.mediaItems.length})</h4>
                            <div className="flex flex-wrap gap-2">
                              {s.mediaItems.map(m => (
                                <div key={m.id} className="p-2 bg-gray-800 rounded-lg border border-gray-700 flex items-center gap-2 group cursor-pointer" onClick={() => {
                                  const win = window.open();
                                  win?.document.write(`<img src="data:${m.type};base64,${m.data}" style="max-width:100%">`);
                                }}>
                                  <ImageIcon size={14} className="text-emerald-400"/>
                                  <span className="text-[9px] font-bold uppercase text-gray-400">{m.category}</span>
                                </div>
                              ))}
                              {s.mediaItems.length === 0 && <p className="text-[9px] text-gray-600 italic">No evidence logged.</p>}
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <form className="space-y-4" onSubmit={async (e) => {
            ...
            if (files) {
              for (let i = 0; i < files.length; i++) {
                newMedia.push({
                  id: `m-${Date.now()}-${i}`,
                  data: await blobToBase64(files[i]),
                  type: files[i].type,
                  category: cat
                });
              }
